...
if IN_TESTING:
    ...
    # SLACK_BOT_SECRET = os.environ['FOOSBOT_BOT_SECRET'] needs to be added
    ...
    SLACK_BOT_SECRET = os.environ['PBPL_FOOSBOT_SECRET']
    # SLACK_BOT_OAUTH_TOKEN = os.environ.get('PBPL_FOOSBOT_OAUTH_TOKEN')
    foosball_channel = 'C042YTYSDJ5'
    andy_user = 'W01A1T44K2B'
    
# Hardcode to fix slack certificates issue
ssl_context = ssl.create_default_context(cafile=certifi.where())
client = WebClient(token=SLACK_BOT_USER_TOKEN, ssl=ssl_context)
...
def handle_message_events(event, say, ack):
    ack()
    #Return if no parenthesis
    if 'text' in event.keys():
        if (event['text'].find('(')==-1) or (event['text'].find(')')==-1):
            return
...
def handle_game_deletion(say, ack, body, logger):
    ...
    # Acknowledge to let user known computation in progress
    say(
        token = SLACK_BOT_USER_TOKEN,
        text = 'Deletion in progress, recalculating ratings, please wait...'
    )        
